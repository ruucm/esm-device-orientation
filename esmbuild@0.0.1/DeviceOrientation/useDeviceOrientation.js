...
// src/DeviceOrientation/useDeviceOrientation.ts
...
var useDeviceOrientation = () => {
  ...
  const revokeAccessAsync = () => __async(void 0, null, function* () {
    window.removeEventListener("deviceorientation", onDeviceOrientation);
    setOrientation(null);
  });
  const requestAccessAsync = () => __async(void 0, null, function* () {
    console.log("requestAccessAsync!");
    ...
    if (DeviceOrientationEvent.requestPermission && typeof DeviceMotionEvent.requestPermission === "function") {
      let permission;
      try {
        permission = yield DeviceOrientationEvent.requestPermission();
      } catch (err) {
        setError(err);
        return false;
      }
